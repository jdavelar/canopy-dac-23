...
ggplot(., aes(tag, pct, fill = focus)) +
  ...
  labs(title = "Canopy Learning Environments by Student Focus",
       x = "",
       y = "") +
  scale_x_tag() +
...
## Modification 1 - Core tags

```{r core inclusive differences}
inclusive_diff3 <-
full %>% 
  select(school_id, inc = practices_all_courses_designed_for_inclusion, marg = practices_design_marginalized, starts_with("practices")) %>% 
  mutate(inclusive = case_when(
    (inc == 1 | marg == 1) ~ "inclusive",
    (inc == 0 & marg == 0) ~ "not"
  )) %>%
