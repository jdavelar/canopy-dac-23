...
Recreate to match cover essay draft
```{r}
locale2 <- locale_dat %>% 
  ggplot(aes(x = category, fill = waiver_application))+ 
  geom_bar(position = "fill", width = 0.5, color = "white") + 
...
```{r}
ggsave_transcend(locale, "cover_figure10", dir = here("final_products/essay-charts/cover_figures"), fig_width = 12, fig_height = 8)
```
