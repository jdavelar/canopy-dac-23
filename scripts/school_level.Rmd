...
## Add for systems essay -- Guha
```{r}
fig3 <-  confidential %>% 
  select(innovation_initiative_importance) %>% 
  drop_na() %>% 
  ggplot(aes(x ="", fill = as.factor(innovation_initiative_importance)))+ 
  geom_bar(position = "fill", width = .5, color = "white") + 
  coord_polar(theta = "y",start = 2) +
  geom_text(aes(label = scales::percent(..count../tapply(..count.., ..x.. ,sum)[..x..])),
            position = position_fill(vjust = 0.5),
            stat = "count",
  ...
  scale_fill_manual(name="",
   values = transcend_cols,
    ...
    labels = c("Very Important",
                ...
                "Slightly Important"))+
  scale_y_continuous(expand = c(0,0))+
  labs(title = "Schools say district/charter innovation initiatives are important to their work",
       subtitle = "Responses to “How important would you say this initiative is to the \nsuccess of your learning environment’s innovation work?” (N = 62)") +
theme(
  #  plot.title.position = "plot",
   plot.title = element_text(family = "Bebas Neue", color = "black", hjust=0.5),
    #plot.subtitle = element_text(family = "Open Sans", size = rel(0.8)),
...
```{r}
ggsave_transcend(fig3, "systems_figure3", dir = here("final_products/essay-charts/systems_figures"), fig_width = 12, fig_height = 8)
```
...
## Edits for systems essay -- Guha

```{r}
fig2 <- support_comparison %>%
  ggplot(aes(x=reorder(name, percent_yes), y = percent_yes, fill = factor(innovation)))+
  geom_bar(position = "dodge",stat = "identity") +
  labs(title = "Schools in district/charter innovation initiatives receive more central office support ",
       subtitle = "Responses to “Does your central office provide any of the following related to your innovation work?” \nfrom schools that reported being part of system-wide innovation initiatives compared to those that did not.",
       y = "Percentage of public schools in Canopy",
       x = "",
      fill = "") +
  scale_x_discrete(labels = support_labels) +
  scale_y_continuous(limits = c(0,100), n.breaks = 5,labels = function(x) paste0(x, "%"), expand = c(0,0)) +
...
```{r}
ggsave_transcend(fig2, "systems_figure2", dir = here("final_products/essay-charts/systems_figures"), fig_width = 12, fig_height = 8)
```
