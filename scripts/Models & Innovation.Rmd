  ...
  scale_fill_manual(name="",
   values = transcend_cols2,
    ...
    labels = c("No", 
               ...
               "Yes, we considered but did not apply", 
               "Not sure",
               "NA")) +
...
full %>% 
  ...
  drop_na() %>% 
  ggplot(aes(x = school_state, fill = as.factor(waiver_application)))+ 
  geom_bar(position = "fill", width = 1, color = "white") + 
#  coord_polar(theta = "y", start = 180) +
...
model %>%
  ...
  filter(count == "1") %>% 
  drop_na() %>% 
  ggplot(aes(x = category, fill = as.factor(waiver_application)))+ 
...
full %>%
  ...
  drop_na() %>% 
    mutate(
           political_impact = factor(political_impact, levels = c("5", "4", "3", "2", "1"))) %>%
  ggplot(aes( x=(school_state), fill=as.factor(political_impact)))+ 
  geom_bar(stat= "count", 
           position = "fill")+
...
model %>%
  ...
  scale_fill_manual(values=c("#FFA630", "#FFDE42", "#D1D3D4", "#99C24D","#218380"),
   ...
   labels=c("Strongly Disagree",
            ...
            "Strongly Agree"),
   name="")+
  labs(title = "By Locale: To what extent do you agree with the following statement? \n“Political debates are detracting from our learning environment’s ability to stay focused on our mission.”") +
  coord_flip() +
  theme_2()
```
