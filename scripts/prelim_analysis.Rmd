...
ggplot(level, aes(`School level`, pct, fill = `School level`)) +
  ...
  labs(title = "Canopy Learning Environments by Level",
       ...
       y = "") +
  theme(legend.position = "none") +
    geom_text(aes(label = scales::label_percent(accuracy = 1)(level$pct)), 
            nudge_y = 3, 
            color = "black",
            fontface = "bold",
            size = 5,
...
```{r}
#I intended to make a pie chart to show percentages, but realized our schools do not add up to 100%
#pie chart for report
#pie <- 
#   dat %>% 
...
#          "Middle" = grades_middle,
#          "High" = grades_high) %>% 
#   pivot_longer(cols = c(Prekindergarten, Elementary, Middle, High),
#                names_to = "level",
#                values_to = "n") %>% 
#   mutate(level = factor(level,
#                            levels = c("Prekindergarten", "Elementary", "Middle", "High"),
#                            labels = c("Prekindergarten", "Elementary", "Middle", "High")))
# ggplot(level, aes(x ="", y = -N, fill = `School level`)) + 
...
# type <- 
  ...
  theme(legend.position = "none") +
    geom_text(aes(label = signif(N)), 
            ...
            size = 5,
            family = "sans")
# ggsave("finding-2.png", plot = type, path = here("final_products", "draft-findings"),
#        width = 12, height = 8, units = "in")
```
