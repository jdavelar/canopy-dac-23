...
top_prac <- 
  ...
  theme(legend.position = c(.8,.75), 
    ...
    # geom_text(aes(label = scales::label_percent(accuracy = 1)(tot_pct)),
    #         nudge_y = .01,
    #         vjust = 0,
    #         color = transcend_cols[3],
    #         fontface = "bold",
...
top_prac2 <- 
  ...
  ungroup() %>% 
  slice_head(., n = 40) %>% 
  ggplot(., aes(Practice, pct, fill = Time)) +
  geom_col() +
  bar_y_scale_percent +
  ...
  guides(fill = guide_legend(reverse = TRUE))
    ...
    #         family = "sans")
top_prac2
ggsave("finding-8b-V2.png", plot = top_prac2, path = here("final_products", "draft-findings"),
       width = 8, height = 6, units = "in")
```
